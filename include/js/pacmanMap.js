...
Pacman.Map = function (size) 
{
        ...
        for (i = 0; i < height; i += 1) 
        {
		    for (j = 0; j < width; j += 1) 
            {
                if (map[i][j] === Pacman.PILL) 
                {
                    ...
                    ctx.fillStyle = "#ddd"; // background color big pills
		            ctx.fillRect((j * blockSize), (i * blockSize), 
                                 blockSize, blockSize);

                    ctx.fillStyle = "#c187b3"; // color of the big pills
    ...
    function drawBlock(y, x, ctx) 
    {
        ...
        {
            ctx.fillStyle = "#ddd"; // background color of the small pills
		    ctx.fillRect((x * blockSize), (y * blockSize), blockSize, blockSize);

            if (layout === Pacman.BISCUIT) 
            {
                ctx.fillStyle = "#fca1e5"; // color of the small pills
